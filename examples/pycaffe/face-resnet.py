...
def conv_factory(bottom, ks, nout, stride=1, pad=0, group=1):
...
def conv_factory_relu(bottom, ks, nout, stride=1, pad=0, group=1):
    conv = L.Convolution(bottom, kernel_size=ks, stride=stride,
                                 num_output=nout, pad=pad, group=group,
                                 bias_term=False, weight_filler=dict(type='msra'))
    batch_norm = L.BatchNorm(conv, in_place=True, param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0)])
...
def conv_factory_relu_inverse_no_inplace(bottom, ks, nout, stride=1, pad=0,group=1):
...
def residual_factory1(bottom, num_filter):
    bottleneck1 = conv_factory_relu_inverse_no_inplace(bottom, 1, num_filter / 2, 1, 0)
    conv = conv_factory_relu_inverse_no_inplace(bottleneck1, 3, num_filter / 2, 1, 1, 16)
    bottleneck2 = conv_factory_relu_inverse(conv, 1, num_filter, 1, 0)
    addition = L.Eltwise(bottom, bottleneck2, operation=P.Eltwise.SUM)
    return addition

def residual_factory_proj(bottom, num_filter, stride=2):
    batch_norm = L.BatchNorm(bottom, in_place=True, param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0)])
    scale = L.Scale(batch_norm, bias_term=True, in_place=True)
    bottleneck1 = conv_factory_relu(scale, 1, num_filter / 2, stride, 0)
    conv = conv_factory_relu_inverse_no_inplace(bottleneck1, 3, num_filter / 2, 1, 1, 16)
    bottleneck2 = L.Convolution(conv, kernel_size=1, stride=1,
                                num_output=num_filter, pad=0, weight_filler=dict(type='msra'))
    proj = L.Convolution(scale, kernel_size=1, stride=stride,
                                num_output=num_filter, pad=0, weight_filler=dict(type='msra'))
    addition = L.Eltwise(bottleneck2, proj, operation=P.Eltwise.SUM)
    return addition
