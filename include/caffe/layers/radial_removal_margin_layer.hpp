...
class RadialRemovalMarginLayer : public Layer<Dtype> {
 public:
  explicit RadialRemovalMarginLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  ...
  virtual inline const char* type() const { return "RadialRemovalMargin"; }
 ...
 protected:
  ...
  virtual Dtype CalcSoftNegativeMean(const vector<Blob<Dtype>*>& bottom);
  ...
  Blob<Dtype> sum_exp_negative_logits_;
  Blob<Dtype> log_sum_exp_negative_logits_;
