...
namespace caffe {
  ...
  void BNLayer<Dtype>::multicast_gpu(int N, int C, int S,
  ...
  // y[c] = sum x(.,c,...)
  template <typename Dtype>
  void BNLayer<Dtype>::compute_sum_per_channel_gpu(int N, int C, int S,
                                                          const Dtype *x, Dtype *y) {
  ...
  // y[c] = mean x(.,c,...)
  template <typename Dtype>
  void BNLayer<Dtype>::compute_mean_per_channel_gpu(int N, int C, int S,
                                                           const Dtype *x, Dtype *y) {
    Dtype F = 1.0 / (N*S);
  ...
  template void BNLayer<float>::multicast_gpu(int N, int C,
                                                     int S, const float *x, float *y);
  template void BNLayer<float>::compute_sum_per_channel_gpu(int N, int C,
                                                                   int S, const float *x, float *y);
  template void BNLayer<float>::compute_mean_per_channel_gpu(int N, int C,
                                                                    int S, const float *x, float *y);

  template void BNLayer<double>::multicast_gpu(int N, int C,
                                                      int S, const double *x, double *y);
  template void BNLayer<double>::compute_sum_per_channel_gpu(int N, int C,
                                                                    int S, const double *x, double *y);
  template void BNLayer<double>::compute_mean_per_channel_gpu(int N, int C,
                                                                     int S, const double *x, double *y);
