...
void FeatureDecayLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  incay_threshold_ = this->layer_param_.feature_decay_loss_param().incay_threshold();
...
void FeatureDecayLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    if (moving_average_norm[0] > decay_threshold_) {
      caffe_cpu_axpby(num, loss_weight[0] / num, bottom[0]->cpu_data(), Dtype(0), bottom[0]->mutable_cpu_diff());
    }
    else if (moving_average_norm[0] < incay_threshold_) {
      caffe_cpu_axpby(num, -loss_weight[0] / num, bottom[0]->cpu_data(), Dtype(0), bottom[0]->mutable_cpu_diff());
    }
    else {
