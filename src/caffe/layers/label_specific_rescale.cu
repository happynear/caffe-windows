...
namespace caffe {
  ...
  __global__ void LabelSpecificPowerPositiveForward(const int n, const int dim, const Dtype* in, const Dtype* label,
    ...
    CUDA_KERNEL_LOOP(index, n) {
      ...
      int k = (int)floor(acos(in[index * dim + l]) * 4 / M_PI);
      out[index * dim + l] = positive_weight * in[index * dim + l];
      out[index * dim + l] += powf(-1, k) * (8 * powf(in[index * dim + l], 4) - 8 * powf(in[index * dim + l], 2) + 1) - 2 * k;
      if (bias_fix) {
        out[index * dim + l] -= 1;
      }
  ...
  __global__ void LabelSpecificPowerPositiveBackward(const int n, const int dim, const Dtype* in, const Dtype* label,
    ...
    CUDA_KERNEL_LOOP(index, n) {
      int l = static_cast<int>(label[index]);
      int k = (int)floor(acos(in[index * dim + l]) * 4 / M_PI);
      out[index * dim + l] = in[index * dim + l] / (1 + positive_weight);
      out[index * dim + l] *= powf(-1, k) * (32 * powf(bottom_data[index * dim + l], 3) - 16 * bottom_data[index * dim + l]) + positive_weight;
    }
  }
